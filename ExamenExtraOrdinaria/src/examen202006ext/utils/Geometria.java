package examen202006ext.utils;

import java.awt.Point;
import java.awt.geom.Point2D;

/** Clase utilitaria con funciones geométricas diversas
...
public class Geometria {
    ...
    /** Devuelve la orientación de un punto respecto a un segmento
     * @param s1	Punto inicial del segmento
     * @param s2	Punto final del segmento
     * @param p	Punto de orientación
     * @return	1 si el punto p está hacia la derecha de la recta formada por el segmento direccional s1-s2, -1 si está a la izquierda, 0 si está justo en el segmento
     */
    public static int orientacion(Point s1, Point s2, Point p) {
        int val = (s2.y - s1.y) * (p.x - s2.x) - (s2.x - s1.x) * (p.y - s2.y);
        if (val == 0) return 0;
	...
	/** Calcula el ángulo del vector entre dos puntos
	 * @param p1	Punto inicial
	 * @param p2	Punto final
	 * @return	Ángulo entre ambos en radianes
	 */
